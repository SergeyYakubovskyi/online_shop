(() => {
    ...
    function rangeInit() {
        ...
        if (priceSlider) {
            ...
            function setPriceValues() {
                let priceStartValue = priceStart.value || 300;
                let priceEndValue = priceEnd.value || 5e4;
    ...
    function initSliders() {
        ...
        if (document.querySelector(".product__slider--2")) new swiper_core_Swiper(".product__slider--2", {
...
*/
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (searchInput && dropdown && overlay) {
            ...
            document.addEventListener("click", (event => {
                if (!dropdown.contains(event.target) && !searchInput.contains(event.target)) toggleDropdown(false);
            }));
            function toggleDropdown(show) {
                dropdown.classList.toggle("active-ov", show);
                overlay.classList.toggle("active-ov", show);
            }
            searchInput.addEventListener("input", (() => {
                const inputText = searchInput.value.trim().toLowerCase();
                const startsWithSho = inputText.startsWith("sho");
                dropdown.classList.toggle("active-block", startsWithSho);
                errorMessage.style.display = startsWithSho || !inputText ? "none" : "block";
            }));
        }
        const filterMenuIcon = document.querySelector(".shop-item__icon");
        if (filterMenuIcon) filterMenuIcon.addEventListener("click", (() => {
            const filterMenu = document.querySelector(".filter");
            if (filterMenu) filterMenu.classList.toggle("filter-open");
        }));
    }));
    gsapWithCSS.registerPlugin(ScrollTrigger_ScrollTrigger);
    gsapWithCSS.to(".product__image--anim", {
        scrollTrigger: {
    ...
    gsapWithCSS.utils.toArray(".product__link--anim").forEach((link => {
        gsapWithCSS.from(link, {
            scrollTrigger: {
                trigger: link,
                start: "top bottom",
                end: "top top",
    ...
    gsapWithCSS.utils.toArray(".product__description--anim").forEach((description => {
        gsapWithCSS.from(description, {
            scrollTrigger: {
                trigger: description,
                start: "top bottom",
                end: "top top",
